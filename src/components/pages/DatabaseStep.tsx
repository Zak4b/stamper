...
export default function DatabaseStep({ onSamplePDFSelected }: DatabaseStepProps) {
	return (
		<>
			...
			<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
				...
				<label className="cursor-pointer inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
					...
					<input type="file" accept=".pdf" className="hidden" onChange={(e) => e.target.files?.[0] && onSamplePDFSelected(e.target.files[0])} />
				</label>
			</div>
		</>
	);
